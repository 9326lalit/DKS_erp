...
export default function BanaGRNListPage() {
  ...
  const [editGRN, setEditGRN] = useState<BanaGRN | null>(null);
  const [deleteConfirmOpen, setDeleteConfirmOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<BanaGRN | null>(null);
  ...
  const updateMutation = useMutation({
    mutationFn: (data: BanaGRN) => banaApiService.updateGRN(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["bana-grns"] });
      queryClient.invalidateQueries({ queryKey: ["bana-pos"] });
      toast.success("Bana GRN updated successfully.");
      setEditGRN(null);
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => banaApiService.deleteGRN(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["bana-grns"] });
      queryClient.invalidateQueries({ queryKey: ["bana-pos"] });
      toast.success("Bana GRN deleted.");
  ...
  const handleEditClick = (grn: BanaGRN) => {
    setEditGRN(grn);
    setViewGRN(null);
  };

  const handleDeleteClick = (grn: BanaGRN) => {
    setDeleteTarget(grn);
  ...
  return (
    <PageContainer>
      ...
      <MasterToolbar
        ...
        onCreateClick={() => window.location.href = "/dashboard/bana/goods-receipt/new"}
        exportTitle="Bana GRNs"
      ...
      <MasterDialog
        ...
        title="Goods Receipt Note Details (Bana)"
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="text-sm font-bold flex items-center gap-2 text-destructive">
              <Trash2 className="h-4 w-4" />Delete Bana Goods Receipt Note?
            </AlertDialogTitle>

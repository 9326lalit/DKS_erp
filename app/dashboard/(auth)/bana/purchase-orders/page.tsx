...
export default function BanaPurchaseOrdersPage() {
  const queryClient = useQueryClient();
  const [searchValue, setSearchValue] = useState("");
  const [selectedFilters, setSelectedFilters] = useState<Record<string, string>>({ status: "all" });
  const [viewPO, setViewPO] = useState<BanaPO | null>(null);
  const [deleteConfirmOpen, setDeleteConfirmOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<BanaPO | null>(null);

  const { data: pos = [], isLoading } = useQuery({ queryKey: ["bana-pos"], queryFn: () => banaApiService.getPOs() });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => banaApiService.deletePO(id),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["bana-pos"] }); toast.success("Bana PO deleted."); setDeleteConfirmOpen(false); setDeleteTarget(null); }
  });
  ...
  const columns: TableColumn<BanaPO>[] = [
  ...
  return (
    <PageContainer>
      <PageHeader
        title="Bana Purchase Orders"
        description="Raise and track Purchase Orders for Bana (Weft Yarn). Document series: BANA-PO-YYYY-NNNN."
        breadcrumbs={[{ title: "Dashboard", href: "/dashboard/default" }, { title: "Bana (Weft)" }, { title: "Purchase Orders" }]}
        actions={
          <Link href="/dashboard/bana/purchase-orders/new">
            <Button size="sm" className="h-9 gap-1.5 cursor-pointer"><Plus className="h-4 w-4" /> Create Bana PO</Button>
          </Link>
        }
      />
      ...
      <MasterToolbar
        ...
        createLabel="Create Bana PO" onCreateClick={() => window.location.href = "/dashboard/bana/purchase-orders/new"}
        exportTitle="Bana POs" selectedFilters={selectedFilters}
        onFilterChange={(key, val) => setSelectedFilters(p => ({ ...p, [key]: val }))}
        onClearFilters={() => { setSearchValue(""); setSelectedFilters({ status: "all" }); }}
        filters={[{ key: "status", placeholder: "PO Status", options: [{ label: "Open", value: "Open" }, { label: "Partially Received", value: "Partially Received" }, { label: "Closed", value: "Closed" }] }]}
      />
      ...
      <MasterDialog isOpen={!!viewPO} onClose={() => setViewPO(null)}
        ...
        {viewPO && (
          <div className="space-y-4 text-xs">
            ...
            <div className="grid grid-cols-2 gap-3 border-b pb-3 border-border/10">
              <div><span className="text-muted-foreground block">Supplier</span><span className="font-bold">{viewPO.purchaseFromName}</span></div>
              <div><span className="text-muted-foreground block">Buyer</span><span className="font-semibold">{viewPO.purchaseToName}</span></div>
            </div>
            <div className="grid grid-cols-2 gap-3 border-b pb-3 border-border/10">
              <div><span className="text-muted-foreground block">Item / Quality</span><span className="font-semibold">{viewPO.itemName}</span></div>
            ...
            <div className="grid grid-cols-3 gap-3 border-b pb-3 border-border/10">
              <div><span className="text-muted-foreground block">Bags Ordered</span><span className="font-bold">{viewPO.totalBagsOrdered}</span></div>
              <div><span className="text-muted-foreground block">Per Bag Wt.</span><span className="font-semibold">{viewPO.perBagWeightKg} KG</span></div>
            ...
            <div className="bg-muted/10 rounded-lg border border-border/10 p-3 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="flex justify-between"><span className="text-muted-foreground">Rate per KG</span><span className="font-semibold">₹{viewPO.ratePerKg}</span></div>
              ...
              <div className="flex justify-between font-bold text-sm border-t border-border/20 pt-2">
                <span>Net Payable</span><span className="text-foreground text-base">₹{viewPO.netPayable.toLocaleString()}</span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><span className="text-muted-foreground block">Delivery Address</span><p className="font-semibold">{viewPO.deliveryAddress}</p></div>
            </div>
            <div className="flex justify-end pt-2"><Button variant="outline" onClick={() => setViewPO(null)}>Close</Button></div>
          </div>
        )}
      </MasterDialog>
      ...
      <AlertDialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="text-sm font-bold flex items-center gap-2 text-destructive"><Trash2 className="h-4 w-4" />Delete Bana PO?</AlertDialogTitle>
            <AlertDialogDescription className="text-xs">Delete <strong>{deleteTarget?.poNumber}</strong>?</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

...
import { ArrowLeft, Printer, Building2, ShieldCheck } from "lucide-react";
...
export default function BanaInvoiceDetailPage({ params }: { params: { id: string } }) {
    ...
    return (
      <PageContainer>
        <div className="text-center py-16">
          <h2 className="text-xl font-bold text-foreground">Bana Invoice Not Found</h2>
          <p className="text-sm text-muted-foreground mt-2">The requested Purchase Invoice ID does not exist.</p>
          <Button asChild className="mt-4">
            <Link href="/dashboard/bana/invoices">Return to Bana Invoices</Link>
          </Button>
        </div>
      </PageContainer>
    );
  ...
  const hsnCode = (pi as any).hsnCode || "5402";
  ...
  return (
    <PageContainer>
      <PageHeader
        title={`Bana Purchase Invoice: ${pi.piNumber}`}
        description={`Tax Purchase Invoice for Bana (Weft Yarn) received from ${pi.supplierName}.`}
        breadcrumbs={[
          { title: "Dashboard", href: "/dashboard/default" },
          { title: "Bana Module", href: "/dashboard/bana/invoices" },
          { title: "Invoices", href: "/dashboard/bana/invoices" },
          { title: pi.piNumber }
        ]}
        actions={
        ...
        <CardContent className="p-8 space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-start border-b border-border/40 pb-6 gap-4">
            ...
            <div className="text-left sm:text-right space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                Payment Status:{" "}
                <Badge variant="outline" className={`${paymentStatusColors[pi.paymentStatus]} font-bold text-[10px]`}>
                  {pi.paymentStatus}
                </Badge>
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-6 text-[11px] text-muted-foreground pt-4">
            <div>
              ...
              <p>2. Subject to Ichalkaranji Jurisdiction.</p>
            </div>

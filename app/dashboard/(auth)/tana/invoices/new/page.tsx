...
const schema = z.object({
  ...
  totalWeightKg: z.number().min(0.1),
...
export default function NewTanaPIPage() {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      linkedGRNId: "", supplierInvoiceNo: "", supplierInvoiceDate: new Date().toISOString().split("T")[0],
      itemDescription: "", totalWeightKg: 0, ratePerKg: 0,
      cgstPercent: 6, sgstPercent: 6, roundOff: 0,
      paymentTermsDays: 30, paymentStatus: "Pending"
    }
  });
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Tana Purchase Invoice created!");
  ...
  const onSubmit = (values: FormValues) => {
    ...
    mutation.mutate({
      id: `TANA-PI-ID-${Date.now()}`,
      piNumber: `TANA-PI-${year}-${String(seq).padStart(4, "0")}`,
      linkedGRNId: selectedGRN.id,
      linkedGRNNumber: selectedGRN.grnNumber,
      linkedPOId: selectedGRN.linkedPOId,
      linkedPONumber: selectedGRN.linkedPONumber,
  ...
  return (
    <PageContainer>
      <PageHeader
        title="Create Tana Purchase Invoice"
        description="Generate a Purchase Invoice against a Tana GRN with full CGST/SGST breakup, round-off and due date. Series: TANA-PI-YYYY-NNNN."
        breadcrumbs={[{ title: "Dashboard", href: "/dashboard/default" }, { title: "Tana Invoices", href: "/dashboard/tana/invoices" }, { title: "New Invoice" }]}
          ...
          <CardContent className="pt-5 space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                ...
                <Input {...form.register("supplierInvoiceNo")} placeholder="e.g. YJS/2026/0042" />
            ...
            <div className="space-y-1.5">
              ...
              <Textarea rows={2} {...form.register("itemDescription")} placeholder="e.g. 40s Cotton Warp Yarn — 2500 KG (50 Bags)" />
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-5 space-y-4">
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="space-y-1.5">
                ...
                <Input type="number" step="0.01" {...form.register("totalWeightKg", { valueAsNumber: true })} />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs font-semibold">Taxable Amount (Auto)</Label>
                <div className="h-10 px-3 flex items-center rounded-md border bg-muted/30 text-sm font-bold">{taxableAmount > 0 ? `₹${taxableAmount.toLocaleString("en-IN")}` : "—"}</div>
              </div>
            </div>
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <Input type="number" step="0.01" {...form.register("roundOff", { valueAsNumber: true })} placeholder="0.00" />
              </div>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="space-y-1.5">
                ...
                <Input type="number" {...form.register("paymentTermsDays", { valueAsNumber: true })} placeholder="30" />
              </div>
              <div className="space-y-1.5">
